...
# Fill your nubers in the boad below or create a new array to solve the board you give.
...
def print_board(bo):
    ...
    for i in range(len(bo)):
        ...
        for j in range(len(bo[0])):
            ...
            # printing each character with sapces exep for the last
            if j % 3 == 0 and j != 0:
                print(" | ", end="")
            if j == 8:
                # last character is printed withot spaces behind
                # its the last because we started counting at 0
                print(bo[i][j])
            else:
...
def solve(bo):
    # searching for next epmty solt
    slot = next_empty(bo)
    if not slot:
        # return True if there is no emty slot
        return True
    else:
        row, col = slot
    ...
    for i in range(1, 10):
        ...
        if possible(bo, (row, col), i):
            ...
            # starting recursion
            if solve(bo):
                # returns True when the previos returned True
                # This will only activate if the board is full
                return True

            # restting the changed value to 0
            bo[row][col] = 0
    return False

# use this function to generate a new board
# comment and use your own array to solve manual preset boards.
